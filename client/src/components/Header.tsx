...
export default function Header() {
  const { user, loggedIn, logout } = useAuth(); // Use auth context
        ...
        {isOpen && (
          ...
          >
            <nav className="flex flex-col items-center py-4 gap-4">
              <Link href="/gigs" className="text-lg hover:text-blue-600 dark:hover:text-blue-400 transition duration-200" onClick={() => setIsOpen(false)}>
                Gigs
              </Link>
              {loggedIn && (
                <>
                  <Link href="/orders" className="text-lg hover:text-blue-600 dark:hover:text-blue-400 transition duration-200" onClick={() => setIsOpen(false)}>
                    Orders
                  </Link>
                  <Link href="/dashboard" className="text-lg hover:text-blue-600 dark:hover:text-blue-400 transition duration-200" onClick={() => setIsOpen(false)}>
                    Dashboard
                  </Link>
                  <Link href="/profile" className="text-lg hover:text-blue-600 dark:hover:text-blue-400 transition duration-200" onClick={() => setIsOpen(false)}>
                    Profile
                  </Link>
                </>
              )}
